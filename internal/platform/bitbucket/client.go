...
func init() {
	platform.Register("bitbucket", func(cfg any) (platform.Platform, error) {
		c, ok := cfg.(*Config)
		if !ok {
			return nil, fmt.Errorf("bitbucket: expected *Config, got %T", cfg)
		}
		return NewClient(c)
	})
}
...
func (c *Client) do(ctx context.Context, method, path string, body []byte) ([]byte, error) {
	...
	for attempt := range maxRetries {
		...
		if err != nil {
			return nil, fmt.Errorf("bitbucket: executing request: %w", err)
		}

		respBody, err := io.ReadAll(resp.Body)
		resp.Body.Close()
		if err != nil {
			return nil, fmt.Errorf("bitbucket: reading response body: %w", err)
		}
		...
		if resp.StatusCode == http.StatusTooManyRequests {
			...
			select {
			case <-ctx.Done():
				return nil, fmt.Errorf("bitbucket: %w", ctx.Err())
...
func (c *Client) doURL(ctx context.Context, url string) ([]byte, error) {
	var lastErr error
